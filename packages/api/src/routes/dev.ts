...
// Real upstream model IDs live in the environment (see .env) so the tracked
// source carries only BTL Runtime tier names. BTL_WIRE_* map BTL tiers -> models.
function buildLLMConfig(model: string, byokKey?: string, byokProvider?: string): import('@agentr/core').LLMConfig {
  ...
  const flagship = process.env['BTL_WIRE_FLAGSHIP'] ?? process.env['BTL_WIRE_MODEL'] ?? ''
  const fast = process.env['BTL_WIRE_FAST'] ?? flagship
  const code = process.env['BTL_WIRE_CODE'] ?? flagship
  const mini = process.env['BTL_WIRE_MINI'] ?? flagship
  ...
  if (byokKey && (byokProvider === 'byok' || byokProvider === 'openai')) {
    return { provider: 'air', apiKey: byokKey, model: flagship }
  }
  return { provider: 'air', apiKey: airKey, model: modelMap[model] || flagship }
}
...
devRoutes.post('/chat', zValidator('json', z.object({
  ...
  model: z.enum(['max', 'flash', 'codex', 'air', 'gpt4']).default('air'),
  apiKey: z.string().optional(),
  provider: z.enum(['byok', 'openai', '']).optional(),
})), async (c) => {
