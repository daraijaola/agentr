...
// The BTL Runtime gateway only accepts concrete upstream model IDs. Our BTL tier
// IDs (btl-2, deepseek-*) are branding used for the dashboard label, credit
// accounting and plan gating — they are NOT valid gateway models, so sending them
// raw returns HTTP 400 "Unsupported or missing model". We therefore keep the BTL
// tier ID everywhere the product needs it and translate to the real upstream
// model only for the outbound request. The real ID is supplied via the
// BTL_WIRE_MODEL environment variable so it never lives in the tracked source.
// Read at CALL time (not module load) so the value is correct regardless of when
// the process loads its .env — module import ordering can evaluate this file
// before dotenv runs, which would otherwise leave the wire model empty.
function getWireModel(): string {
    return process.env['BTL_WIRE_MODEL'] ?? process.env['LLM_MODEL'] ?? '';
}
// Already a concrete gateway model (any vendor-prefixed id)? -> pass through.
const CONCRETE_MODEL_RE = /^(gpt-|o\d|claude-|deepseek-r|kimi-)/;
function toWireModel(model: string): string {
    if (CONCRETE_MODEL_RE.test(model)) return model;
    // Internal BTL tier alias -> resolve to the configured upstream model.
    return getWireModel() || model;
}

// Models that reject the temperature parameter (resolved upstream ids, from env)
function skipsTemperature(model: string): boolean {
    return (process.env['BTL_SKIP_TEMP_MODELS'] ?? '')
        .split(',').map(s => s.trim()).filter(Boolean).includes(model);
}
const STARTER_TTL_MS = 24 * 60 * 60 * 1000; // 24 hours
const MAX_INPUT_BYTES = 28 * 1024; // 28 KB — BTL Runtime gateway limit
 ...
 *  Some upstream models reject role:"tool" — flatten tool results into role:"user".
...
export class LLMClient {
    ...
    async chat(options) {
        ...
        // `model` = internal BTL tier ID (plan gating + credit accounting).
        // `wireModel` = the real upstream model actually sent (behind the BTL tier).
        ...
        if (isReasoningModel(wireModel)) {
            ...
            // Only send temperature when explicitly configured — some upstream
            // models reject a non-default temperature
            if (this.config.temperature !== undefined)
                if (!skipsTemperature(wireModel)) { body.temperature = this.config.temperature; }
        }
        ...
        // Format 3b: <tool_use>\ntool_name\n{json}\n</tool_use>
        // XML tool-use format — tool name on its own line after opening tag
        ...
        // Some OpenAI-compatible gateways ignore the native tools array
