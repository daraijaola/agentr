...
export const TOOL_NAMES = [
  ...
  `Bots: create_telegram_bot, bot_inline_send, botfather_command`,
...
export const DOMAIN_FLOWS = [
  ...
  `TELEGRAM BOT DEPLOY FLOW — NON-NEGOTIABLE, NO STOPPING: When asked to create and run/host a Telegram bot, always complete ALL steps in ONE turn: (1) create_telegram_bot to register bot with BotFather — note the returned token and @username. (2) workspace_write the bot script (bot.js or bot.py) — write production-quality code with full command handling, error handling, and .env loading. Load the token from process.env.BOT_TOKEN. (3) workspace_write a .env file containing BOT_TOKEN=<token from step 1>. (4) exec_install to install all dependencies (e.g. node-telegram-bot-api or python-telegram-bot). (5) process_start to run the bot 24/7 (use env_file=".env"). (6) process_logs to confirm the bot started without errors. (7) Reply with the bot @username as a clickable link: "t.me/<username>". Never stop between steps, never ask "should I deploy it?".`,
  `TELEGRAM WEB APP FLOW — NON-NEGOTIABLE, NO STOPPING: When asked to build a Telegram Web App (Mini App): (1) create_telegram_bot to register the bot — save the token and @username. (2) workspace_write the full polished HTML/CSS/JS web app with the Telegram SDK (<script src="https://telegram.org/js/telegram-web-app.js"></script>) calling Telegram.WebApp.ready(). (3) serve_static to publish and get the public HTTPS URL. (4) SET MENU BUTTON via Bot API — use code_execute to run: fetch("https://api.telegram.org/bot<TOKEN>/setChatMenuButton", {method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({menu_button: {type:"web_app", text:"Open App", web_app:{url:"<HTTPS_URL>"}}})}).then(r=>r.json()).then(console.log) — this sets the bot's menu button to the web app WITHOUT needing BotFather UI. (5) Reply: "Bot: t.me/<username> — tap the menu button to open". Never stop between steps.`,
