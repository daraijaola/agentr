...
export const DOMAIN_FLOWS = [
  ...
  `TELEGRAM WEB APP FLOW — NON-NEGOTIABLE, NO STOPPING: When asked to build a Telegram Web App (Mini App): (1) create_telegram_bot to register the bot — note the token and @username. (2) workspace_write the full polished HTML/CSS/JS web app that includes the Telegram Web App SDK (<script src="https://telegram.org/js/telegram-web-app.js"></script>) and calls window.Telegram.WebApp.ready(). (3) serve_static to publish the HTML file and get the public HTTPS URL. (4) botfather_command with command="/setmenubutton" and followUp=["@<username>", "url:<https-url>", "Open App"] to attach the web app to the bot's menu button. (5) Reply with both links: "Bot: t.me/<username> | Web App: <url>". Never stop between steps.`,
