...
function resolveLevel(): LogLevel {
  // AGENTR_LOG_LEVEL takes priority
  const explicit = process.env.AGENTR_LOG_LEVEL?.toLowerCase();
  if (explicit && isValidLevel(explicit)) {
    return explicit;
  }

  // Backward compat: AGENTR_LOG=verbose → debug
  if (process.env.AGENTR_LOG === "verbose") {
    return "debug";
  }
 ...
 * Note: pretty mode is controlled by AGENTR_LOG_PRETTY env var only
...
export function initLoggerFromConfig(logging: { level?: string }): void {
  ...
  if (!process.env.AGENTR_LOG_LEVEL && !process.env.AGENTR_LOG) {
