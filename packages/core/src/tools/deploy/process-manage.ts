...
export const processStartTool: Tool = {
  name: "process_start",
  description: "Deploy and start a script as a persistent background process using PM2. The process survives restarts. Use this to deploy Telegram bots or any long-running script from your workspace. IMPORTANT: Always install dependencies via code_execute (bash: pip3 install ... or npm install ...) BEFORE calling this tool, or the process will crash. For static websites, use serve_static instead — it gives users a real public URL without needing a process.",
